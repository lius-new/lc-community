...
pub async fn create(
    ...
    lc_services::articles::article_tags_services::create(payload).await?;
...
pub async fn delete(Path(id): Path<i32>) -> Result<Response<()>, AppError> {
    lc_services::articles::article_tags_services::delete(id).await?;

    Ok(Response::default().success("删除文章标签成功", Some(())))
}

pub async fn toggle_visiable(Path(id): Path<i32>) -> Result<Response<()>, AppError> {
    lc_services::articles::article_tags_services::toggle_visiable(id).await?;
